...
- 「臨時對話」完全不寫檔，只存在記憶體，關掉伺服器就消失（這是故意的設計，
  跟主流AI軟體的「臨時聊天」邏輯一致）

## 已知限制
- 圖片上傳目前對應官方App「送出當下影片畫格」的相同API欄位（screenshot），
  你選的圖片會整張以jpg/png附加上去，沒有額外壓縮或裁切。
- 沒有做任何身份驗證/多使用者機制，這是給你自己在本機用的單人小工具，
...
## 改進方向
1. 加入偽造字幕使請求更真
2. 顯示的地方，程式碼、方程式的展示需做優化
3. 介面優化
4. 圖片上傳後無法在對話中移除，需重整頁面(已知bug)
